...
function fetchSSE(url, options) {
    ...
    return __awaiter(this, void 0, void 0, function () {
        var onMessage, fetchOptions, res, msg, error, parser, body_1, _b, _c, chunk, str, e_1_1;
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    onMessage = options.onMessage, fetchOptions = __rest(options, ["onMessage"]);
                    return [4 /*yield*/, fetch_1.fetch(url, fetchOptions)];
                case 1:
                    res = _d.sent();
                    if (!res.ok) {
                        msg = "ChatGPTAPI error " + (res.status || res.statusText);
                    ...
                    parser = eventsource_parser_1.createParser(function (event) {
                        if (event.type === 'event') {
                            onMessage(event.data);
                        }
                    });
                    if (!!res.body.getReader) return [3 /*break*/, 2];
                    body_1 = res.body;
                ...
                case 2:
                    ...
                    _b = __asyncValues(stream_async_iterable_1.streamAsyncIterable(res.body));
                    _d.label = 3;
